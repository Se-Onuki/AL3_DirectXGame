...
public:
	...
	inline const bool& IsFinish() const { return isFinish_; }

	/// @brief 現在フレームを取得
	/// @return uint32_t 現在フレーム
	inline const uint32_t& GetNowFlame() const { return nowFlame_; }

	/// @brief ゴールフレームを取得
	/// @return uint32_t ゴールフレーム
	inline const uint32_t& GetGoalFlame() const { return goalFlame_; }
