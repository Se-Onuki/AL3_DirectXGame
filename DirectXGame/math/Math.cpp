...
Matrix4x4 MakeScaleMatrix(const Vector3& scale) {
	return Matrix4x4{scale.x, 0, 0, 0, 0, scale.y, 0, 0, 0, 0, scale.z, 0, 0, 0, 0, 1};
}

Matrix3x3 MakeRotateMatrix3x3(const float& theta) {
	return Matrix3x3(cos(theta), sin(theta), 0, -sin(theta), cos(theta), 0, 0, 0, 1);
}

Matrix3x3 MakeTranslateMatrix(const Vector2& translate) {
	return Matrix3x3{1, 0, 0, 0, 1, 0, translate.x, translate.y, 1};
}
...
Matrix3x3 MakeAffineMatrix(const Vector2& scale, const float& theta, const Vector2& translate) {
	return Matrix3x3{
	    scale.x * (float)cos(theta),
	    scale.x * (float)sin(theta),
	    0,
	    scale.y * (float)-sin(theta),
	    scale.y * (float)cos(theta),
	    0,
	    translate.x,
	    translate.y,
	    1};
...
Matrix3x3 MakeOrthographicMatrix(const Vector2& LeftTop, const Vector2& RightBottom) {
	return {
	    2.0f / (RightBottom.x - LeftTop.x),
	    0,
	    0,
	    0,
	    2.0f / (LeftTop.y - RightBottom.y),
	    0,
...
Matrix3x3 MakeViewportMatrix(const Vector2& LeftTop, const Vector2& RightBottom) {
	return {
	    (RightBottom.x - LeftTop.x) / 2.0f,
	    0,
	    0,
	    0,
	    -(RightBottom.y - LeftTop.y) / 2.0f,
	    0,
	    (RightBottom.x - LeftTop.x) / 2.0f + LeftTop.x,
	    (RightBottom.y - LeftTop.y) / 2.0f + LeftTop.y,
	    1};
}
